...
param_SLIC = []
for sl in itertools.product([100, 500, 1000, 2000], [0.01, 0.1, 1, 10, 100], [0, 5, 10, 15]):
    param_SLIC.append(sl)

# create a dictionary for the different sensors:

sensors = {
    "R·G·B": ["rgz.tif", "train/", "train/", "./"]
    }

methods = {
    "SLIC": [SLIC_object_creation, param_SLIC, "SLIC/"]
    }

classifiers = {
    # "rf": [RandomForestClassifier(n_estimators=1000),
    #        dict(rf__criterion=["gini", "entropy"],
    #             rf__max_features=["auto", "sqrt", "log2", None]
    #             ),
    #        "rf"],
    "mlp": [MLPClassifier(max_iter=300),
            dict(mlp__hidden_layer_sizes=[(14,), (14, 14)],
                 mlp__activation=["identity", "logistic", "tanh", "relu"],
                 mlp__solver=["lbfgs", "sgd", "adam"],
                 mlp__learning_rate_init = np.logspace(-3, 0, 4),
                 mlp__alpha = np.logspace(-4, 0, 5)
                 ),
            "mlp"]
    }
...
for k, v in sensors.items():
    ...
    for s, d in methods.items():
        print("Segmentation algorithm = " + s)
        for c, m in classifiers.items():
            print("classifier = " + c)
            # create output text file
            z = open(v[3] + m[2]+"_log.csv", 'w')
            # declare classifier
            classifier = m[0]
            # build pipeline
            steps = [(c, classifier)]
            parameters = m[1] # these are the parameters for the classifier
            pipe = Pipeline(steps)
            cva = GridSearchCV(pipe, param_grid=parameters)
            # run the segmentation method
            for p in range(len(d[1])):
                print("Working on " + str(p+1) + " of " + str(len(d[1])) + " parameters")
                print("Segmentation parameters: " + str(d[1][p]))
                start_time = timer()
                test = d[0](img, training_path, d[1][p][0], d[1][p][1], d[1][p][2])
                end_time = timer()
                # fit the classifier using current segmentation method
                if test is not None:
                    ...
                    cva.fit(X_train, test['tr_labels'])
                    predicted = cva.predict(X_test) # this takes the best estimator and parameters
                    ...
                    print("Best parameters: " + str(cva.best_params_))

                    print(k + ',' + s + ',' + c + ',' + str(d[1][p][0]) + ',' + str(d[1][p][1]) + ',' +
                          str(d[1][p][2]) + ',' + str(len(test['objects'])) + ',' +
                          str(score) + ',' + str(elapsed_time) + ',' + str(cva.best_params_) + "\n", end="", file=z)
                else:
                    print("Did run using current segmentation parameters. Multiple classes were alloted to the same object.")
